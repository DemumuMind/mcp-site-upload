...
function buildQueries({ topic, angle, }: {
    topic: string;
    angle?: string;
}): {
    query: string;
    ...
    const additionalQueries = [
        ...
        normalizedAngle ? `${normalizedTopic} ${normalizedAngle} evidence-based` : "",    ].filter((queryCandidate) => queryCandidate.length > 0);
    return {
        query: primaryQuery,
        additionalQueries,
    };
...
function calculateRelevanceScore(source: Pick<DeepResearchSource, "title" | "snippet" | "publishedDate">, queryTokens: string[]): number {
...
async function searchWithExa({ query, additionalQueries, startPublishedDate, maxCandidates, }: {
    query: string;
    additionalQueries: string[];
...
export async function runDeepResearchWorkflow(input: DeepResearchInput): Promise<DeepResearchPacket> {
    ...
    const querySet = buildQueries({ topic, angle });
    ...
    const averageRelevanceScore = selectedSources.reduce((acc, source) => acc + source.relevanceScore, 0) /
    ...
    const relevanceCheck: ResearchVerificationCheck = {
        ...
        details: selectedSources.length >= 3 && averageRelevanceScore >= 6
...
export function buildDraftPostFromResearch({ packet, slug, titleEn, tags, }: {
    ...
    const keyPointLinesEn = packet.keyPoints.map((point) => `${point.title}. ${point.summary} Supporting sources: ${point.supportingSourceIds.join(", ")}.`);
    const sourceBullets = packet.sources.map((source) => `${source.title} - ${source.url} (${source.publishedDate.slice(0, 10)})`);
