...
function isMissingTableError(message: string): boolean {
    return (message.includes("Could not find the table 'public.blog_posts'") ||
...
function normalizeStoredBlogPost(post: BlogPost): BlogPost {
    ...
    return {
        ...post,
        readTimeMinutes: typeof post.readTimeMinutes === "number" && Number.isFinite(post.readTimeMinutes)
...
function fromTableRow(row: SupabaseBlogPostRow): BlogPost | null {
    ...
    return {
        ...
        readTimeMinutes: typeof row.read_time_minutes === "number" && Number.isFinite(row.read_time_minutes)
...
export async function getBlogPostsFromSupabase(): Promise<BlogPost[]> {
    ...
    try {
        const { data, error } = await supabaseClient
            .from("blog_posts")
            .select("slug, tags, published_at, updated_at, read_time_minutes, featured, cover_image, locale, research")
...
async function savePostToStorage(adminClient: SupabaseClient, input: PersistedBlogPostInput): Promise<void> {
    ...
    const { error } = await adminClient.storage.from(BLOG_STORAGE_BUCKET).upload(objectPath, JSON.stringify(payload, null, 2), {
