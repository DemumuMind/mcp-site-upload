...
function checkVerification(packetId: string, checks: ResearchVerificationCheck[]) {
    ...
    if (failedCheck) {
        throw new Error(`Research packet ${packetId} failed verification at "${failedCheck.round}": ${failedCheck.details}`);
    }
}
...
export async function createBlogPostFromResearch(input: CreateBlogPostFromResearchInput): Promise<BlogDraftCreationResult> {
    ...
    if (runningOnVercel) {
        throw new Error("Supabase storage is required for production blog automation. Configure SUPABASE_SERVICE_ROLE_KEY and run blog_posts migration.");
    }
    ...
    try {
        ...
        if (errorCode === "EROFS" || errorCode === "EPERM") {
            throw new Error("Current environment is read-only. Run blog automation in a writable environment to persist files.");
        }
        throw error;
    }
    return {
