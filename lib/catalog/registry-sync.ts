...
function inferAuthType(server: RegistryServer, source: string): AuthType {
  ...
  const hasSecretEnvironmentVariables = (server.packages ?? []).some((entry) =>
    (entry.environmentVariables ?? []).some((environmentVariable) => Boolean(environmentVariable.isSecret)),
  );
...
function toSyncPayload(record: RegistryRecord): SyncRowPayload | null {
  ...
  const description =
    toSafeString(server.description) ||
  ...
  const officialStatus =
    toSafeString(record._meta?.["io.modelcontextprotocol.registry/official"]?.status).toLowerCase();
...
function getModerationPatternMatch(
  payload: SyncRowPayload,
  matchers: ModerationPatternMatcher[],
): string | null {
  const source = [payload.slug, payload.name, payload.description, payload.repo_url ?? ""].join(" ");
...
async function fetchExistingRows(
  ...
  slugs: string[],
): Promise<Map<string, ExistingServerRow>> {
...
async function upsertRows(
  adminClient: SupabaseClient,
  rows: SyncRowPayload[],
  existingBySlug: Map<string, ExistingServerRow>,
  result: CatalogRegistrySyncResult,
): Promise<void> {
...
async function markStaleRows(
  adminClient: SupabaseClient,
  staleRows: AutoManagedActiveRow[],
  result: CatalogRegistrySyncResult,
): Promise<void> {
...
export async function runCatalogRegistrySync(
  options: CatalogRegistrySyncOptions = {},
): Promise<CatalogRegistrySyncResult> {
  ...
  for (let pageIndex = 0; pageIndex < maxPages; pageIndex += 1) {
    ...
    const nextCursor =
      typeof responsePayload.metadata?.nextCursor === "string"
  ...
  const existingBySlug = await fetchExistingRows(
    adminClient,
