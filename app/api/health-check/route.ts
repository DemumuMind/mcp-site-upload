...
function parseNumberEnv(envName: string, fallbackValue: number, options: NumberEnvOptions): number {
...
async function mapWithConcurrency<TInput, TOutput>(items: readonly TInput[], concurrency: number, mapper: (item: TInput) => Promise<TOutput>): Promise<TOutput[]> {
    if (items.length === 0) {
        return [];
    }
...
async function runHealthCheck(request: NextRequest) {
    ...
    if (!supabaseAdminClient) {
        return NextResponse.json({
            ok: false,
            message: "Supabase admin credentials are not configured. Set NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY.",
        }, { status: 500 });
    }
    const { data, error } = await supabaseAdminClient
        ...
        .eq("status", "active");
    if (error) {
        return NextResponse.json({ ok: false, message: `Failed to query active servers: ${error.message}` }, { status: 500 });
    }
