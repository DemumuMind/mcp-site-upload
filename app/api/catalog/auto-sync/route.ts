...
async function runCatalogAutoSync(request: NextRequest) {
  ...
  const envCleanupStale = parseBooleanEnv(
    "CATALOG_AUTOSYNC_STALE_CLEANUP_ENABLED",
    DEFAULT_STALE_CLEANUP_ENABLED,
  );
  const envQualityFilter = parseBooleanEnv(
    "CATALOG_AUTOSYNC_QUALITY_FILTER_ENABLED",
    DEFAULT_QUALITY_FILTER_ENABLED,
  );
  ...
  const cleanupStale = parseBoolean(
    request.nextUrl.searchParams.get("cleanupStale"),
    envCleanupStale,
  );
  const qualityFilter = parseBoolean(
    request.nextUrl.searchParams.get("qualityFilter"),
    envQualityFilter,
  );
  const allowlistPatterns = mergePatternLists(
    envAllowlistPatterns,
    parsePatternList(request.nextUrl.searchParams.get("allowlist")),
  );
  const denylistPatterns = mergePatternLists(
    envDenylistPatterns,
    parsePatternList(request.nextUrl.searchParams.get("denylist")),
  );
