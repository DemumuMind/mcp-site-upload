...
export default async function DiscordPage() {
    ...
    return (<div className="relative overflow-hidden border-t border-white/10">
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 py-12 sm:px-6 sm:py-14">
        <section className="rounded-3xl border border-indigo-400/20 bg-indigo-950/72 p-6 sm:p-8">
        ...
        <section className="grid gap-4 md:grid-cols-3" aria-label={tr(locale, "Discord channels", "Discord channels")}>
          {communityChannels.map((channel) => (<Card key={channel.name} className="border-white/10 bg-indigo-950/75">
        ...
        <section className="grid gap-4 lg:grid-cols-[1.05fr_0.95fr]">
          <Card className="border-white/10 bg-indigo-950/75">
            ...
            <CardContent className="space-y-2 text-sm text-violet-200">
              {participationRules.map((rule) => (<p key={rule.en} className="rounded-lg border border-white/10 bg-indigo-900/65 px-3 py-2 text-violet-100">
          ...
          <Card className="border-indigo-400/20 bg-[linear-gradient(130deg,rgba(50,38,92,0.55),rgba(8,12,24,0.92))]">
            <CardHeader className="pb-2">
              <CardTitle className="text-xl text-violet-50">{tr(locale, "Why teams join", "Why teams join")}</CardTitle>
            </CardHeader>
