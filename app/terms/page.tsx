...
import { getLegalLastUpdatedValue, legalEmail, legalGmailComposeUrl, termsSections, } from "@/lib/legal-content";
export async function generateMetadata(): Promise<Metadata> {
    const locale = await getLocale();
    return {
        title: tr(locale, "Terms of Service", "Terms of Service"),
        description: tr(locale, "Terms of service and acceptable use policy for DemumuMind MCP.", "Terms of service and acceptable use policy for DemumuMind MCP."),
    };
}
export default async function TermsPage() {
    ...
    return (<div className="mx-auto flex w-full max-w-4xl flex-col gap-8 px-4 py-12 sm:px-6">
      ...
      {termsSections.map((section) => (<section key={section.id} className="space-y-3 border-t border-white/10 pt-5">
      ...
      <section className="space-y-3 border-t border-white/10 pt-5">
        <h2 className="text-2xl font-semibold text-violet-50">
          {tr(locale, "12. Contact", "12. Contact")}
        </h2>
