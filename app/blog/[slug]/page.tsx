...
export async function generateMetadata({ params }: BlogArticlePageProps): Promise<Metadata> {
  ...
  if (!post) {
    return {
      title: tr(locale, "Article not found", "Статья не найдена"),
      description: tr(
        locale,
        "The requested blog article does not exist.",
        "Запрошенная статья блога не найдена.",
      ),
    };
  }
      ...
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-8 px-4 py-10 sm:px-6 sm:py-12">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <Button asChild variant="ghost" className="px-0 text-slate-300 hover:text-white">
            <Link href="/blog">
              ...
              {tr(locale, "Back to blog", "Назад в блог")}
            </Link>
          </Button>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs tracking-[0.1em] text-slate-400 uppercase">
          ...
          <span className="inline-flex items-center gap-1.5">
            ...
            {post.readTimeMinutes} {tr(locale, "min read", "мин чтения")}
          </span>
          {post.updatedAt ? (
            <span>
              {tr(locale, "Updated", "Обновлено")}: {formatDate(post.updatedAt, locale)}
            </span>
          ) : null}
        </div>
