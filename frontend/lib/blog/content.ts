...
function readPostsFromDisk(): BlogPost[] {
    ...
    return fileNames.map((fileName) => {
        ...
        if (normalizeValue(fileSlug) !== normalizeValue(parsed.slug)) {
            throw new Error(`Blog post slug mismatch: file "${fileName}" contains slug "${parsed.slug}".`);
        }
        return {
            ...parsed,
...
const loadedPosts = readPostsFromDisk().sort((left, right) => new Date(right.publishedAt).getTime() - new Date(left.publishedAt).getTime());
