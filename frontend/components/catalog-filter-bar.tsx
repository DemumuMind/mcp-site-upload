...
export function CatalogFilterBar({
  ...
  return (
    <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_170px_150px_130px_auto]">
      <div className="relative">
        <Search className="pointer-events-none absolute top-1/2 left-3 size-4 -translate-y-1/2 text-muted-foreground" />
        <Input
          value={searchQuery}
          onChange={(event) => onSearchQueryChange(event.target.value)}
          placeholder={tr(locale, "Search tools, features, or descriptions...", "Search tools, features, or descriptions...")}
          className="h-10 rounded-xl border-border bg-card pl-9 text-foreground placeholder:text-muted-foreground focus-visible:border-primary/50 focus-visible:ring-primary/20"
        />
      </div>
      ...
      <div className="flex items-center justify-end gap-2">
        ...
        <div className="flex items-center gap-1 rounded-lg border border-border bg-card p-1">
          <Button
            ...
            aria-label={tr(locale, "Grid view", "Grid view")}
          >
          ...
          <Button
            ...
            aria-label={tr(locale, "List view", "List view")}
          >
