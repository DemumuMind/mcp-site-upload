...
export function AuthResetPasswordPanel() {
    ...
    if (!user) {
        ...
        <Button asChild className="mt-6 h-10 rounded-xl bg-violet-50 text-indigo-950 hover:bg-white">
    ...
    if (isCompleted) {
        ...
        <div className="relative p-6 sm:p-10">
          ...
          <div className="mt-5">
            <Button type="button" className="h-10 rounded-xl bg-violet-50 text-indigo-950 hover:bg-white" onClick={() => router.replace("/auth")}>
    ...
    return (<section className="relative overflow-hidden rounded-[2rem] border border-indigo-700/80 bg-[linear-gradient(180deg,rgba(15,23,42,0.96)_0%,rgba(2,6,23,0.98)_100%)] shadow-[0_30px_72px_-50px_rgba(2,6,23,0.95)]">
      ...
      <div className="relative p-6 sm:p-10">
        ...
        <form className="mt-6 grid gap-3 rounded-2xl border border-indigo-700/80 bg-indigo-950/70 p-4 sm:p-5" onSubmit={submitPasswordReset}>
          <div className="space-y-1.5">
            <Label htmlFor="newPassword" className="text-sm text-violet-100">
              {tr(locale, "New password", "New password")}
            </Label>
            <Input id="newPassword" type="password" autoComplete="new-password" required value={values.newPassword} onChange={(event) => updateField("newPassword", event.target.value)} placeholder={tr(locale, `At least ${PASSWORD_MIN_LENGTH} characters`, `At least ${PASSWORD_MIN_LENGTH} characters`)} className="h-11 rounded-xl border-indigo-600 bg-indigo-900/90 text-violet-50 placeholder:text-violet-400 focus-visible:ring-violet-200/35"/>
            <div className="space-y-1.5">
              ...
              <ul className="space-y-1 text-xs">
                {passwordChecklistItems.map((item) => (<li key={item.key} className={`flex items-center gap-2 ${item.passed ? "text-emerald-300" : "text-violet-300"}`}>
          ...
          <div className="space-y-1.5">
            ...
            <Input id="confirmPassword" type="password" autoComplete="new-password" required value={values.confirmPassword} onChange={(event) => updateField("confirmPassword", event.target.value)} placeholder={tr(locale, "Repeat password", "Repeat password")} className="h-11 rounded-xl border-indigo-600 bg-indigo-900/90 text-violet-50 placeholder:text-violet-400 focus-visible:ring-violet-200/35"/>
            {errors.confirmPassword ? (<p className="text-xs text-rose-300">{errors.confirmPassword}</p>) : null}
          </div>

          <Button type="submit" disabled={isPending} className="h-11 rounded-xl bg-violet-50 text-indigo-950 transition hover:bg-white">
