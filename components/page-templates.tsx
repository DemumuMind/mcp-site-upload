...
export function PageHero({ eyebrow, title, description, actions, metrics, badgeTone = "cyan", className }: PageHeroProps) {
  return (
    <section className={cn("space-y-5 rounded-3xl border border-blacksmith bg-black/35 p-6 sm:p-8", className)}>
      {eyebrow ? (
...
export function PageMetric({ label, value, valueClassName, className }: PageMetricProps) {
  return (
    <div className={cn("rounded-2xl border border-blacksmith bg-card p-4", className)}>
...
export function PageSection({ children, className }: PageSectionProps) {
  return <section className={cn("rounded-2xl border border-blacksmith bg-card/90 p-5 sm:p-6", className)}>{children}</section>;
}
