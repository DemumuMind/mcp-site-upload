...
export function PageFrame({ children, variant = "default", className }: PageFrameProps) {
  return (
    <div
      className={cn(
        "relative min-h-screen border-t border-white/10 text-slate-100",
...
export function PageHero({
  ...
  return (
    <section className={cn("space-y-5 rounded-3xl border border-white/12 bg-slate-950/72 p-6 sm:p-8", className)}>
      {eyebrow ? (
        <div
          className={cn(
            "inline-flex w-fit items-center rounded-full border px-3 py-1 text-xs font-semibold tracking-[0.12em] uppercase",
      ...
      <div className="space-y-3">
        <h1 className="text-3xl leading-tight font-semibold tracking-tight text-slate-100 sm:text-5xl">{title}</h1>
        {description ? <p className="max-w-4xl text-sm leading-7 text-slate-300 sm:text-base">{description}</p> : null}
      </div>
...
export function PageMetric({ label, value, valueClassName, className }: PageMetricProps) {
  return (
    <div className={cn("rounded-2xl border border-white/12 bg-slate-900/70 p-4", className)}>
...
export function PageSection({ children, className }: PageSectionProps) {
  return <section className={cn("rounded-2xl border border-white/10 bg-slate-900/55 p-5 sm:p-6", className)}>{children}</section>;
}
