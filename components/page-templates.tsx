...
export function PageHero({ eyebrow, title, description, actions, metrics, badgeTone = "cyan", className }: PageHeroProps) {
  return (
    <section className={cn("home-reveal home-delay-0 space-y-5 rounded-3xl border border-blacksmith bg-black/35 p-6 sm:p-8", className)}>
      {eyebrow ? (
...
export function PageMetric({ label, value, valueClassName, className }: PageMetricProps) {
  return (
    <div className={cn("home-reveal rounded-2xl border border-blacksmith bg-card p-4 transition-transform duration-300 hover:-translate-y-0.5", className)}>
...
export function PageSection({ children, className }: PageSectionProps) {
  return <section className={cn("home-reveal home-delay-1 rounded-2xl border border-blacksmith bg-card/90 p-5 sm:p-6", className)}>{children}</section>;
}
