  ...
  return (
    <section className="space-y-3">
      <h2 className="text-2xl font-semibold tracking-tight text-slate-100">
        {tr(locale, "Related reads", "Похожие материалы")}
      </h2>
      <div className="grid gap-3 md:grid-cols-3">
        {posts.map((post) => {
          ...
          return (
            <Card key={post.slug} className="border-white/10 bg-slate-950/75">
              <CardHeader className="pb-2">
                <CardTitle className="text-base text-slate-100">{localized.title}</CardTitle>
              </CardHeader>
