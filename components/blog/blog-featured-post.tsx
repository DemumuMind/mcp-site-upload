    ...
    <Card className="border-violet-400/20 bg-[linear-gradient(130deg,rgba(53,30,96,0.45),rgba(10,18,35,0.85))]">
      <CardHeader className="pb-2">
        <Badge className="w-fit border-violet-300/40 bg-violet-500/15 text-violet-100">
          ...
          {tr(locale, "Featured article", "Рекомендуемая статья")}
        </Badge>
        <CardTitle className="mt-3 text-2xl text-slate-100 sm:text-3xl">{localized.title}</CardTitle>
      </CardHeader>
          ...
          {tags.map(({ tagSlug, tag }) => {
            return (
              <Badge
                key={tagSlug}
                variant="outline"
                className="border-violet-300/40 bg-violet-500/15 text-violet-100"
              >
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <Button asChild className="bg-blue-500 hover:bg-blue-400">
            <Link href={`/blog/${post.slug}`}>
              {tr(locale, "Open article", "Открыть статью")}
