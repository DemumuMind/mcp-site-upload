...
export function ServerLogo({ mcpServer, className, imageClassName, symbolClassName, showWordmark = false, wordmarkClassName, }: ServerLogoProps) {
    ...
    return (<div className="flex flex-col items-center text-center">
      <div className={cn("inline-flex items-center justify-center overflow-hidden rounded-2xl", hasImageLogo
            ? "border border-white/15 bg-indigo-900/75 shadow-[0_12px_28px_rgba(2,6,23,0.55)] backdrop-blur-[2px]"
            : cn("border border-white/20 bg-gradient-to-br shadow-[0_14px_28px_rgba(2,6,23,0.3)]", fallbackTheme.containerClassName), className)}>
        {hasImageLogo ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={activeLogo} alt={`${mcpServer.name} logo`} loading="lazy" decoding="async" className={cn("h-full w-full object-contain p-2 drop-shadow-[0_6px_12px_rgba(2,6,23,0.4)]", imageClassName)} onError={() => setLogoIndexByKey((current) => ({
                ...current,
                [candidatesKey]: (current[candidatesKey] ?? 0) + 1,
            }))}/>) : (<span className={cn("select-none font-black tracking-tight drop-shadow-[0_2px_8px_rgba(2,6,23,0.45)]", fallbackTheme.symbolClassName, logoStyle.symbolClassName, symbolClassName)}>
            {logoStyle.symbol}
          </span>)}
      </div>
